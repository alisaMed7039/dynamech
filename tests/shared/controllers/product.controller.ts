...
export class Product {
  ...
  public static getRandomProductForAddition(parameters: Partial<ProductModel>): ProductModel {
    return Builder<ProductModel>()
      .productMpn(parameters.productMpn ?? this.getRandomProductNumber())
      .quantity(parameters.quantity ?? faker.number.int({ min: 2, max: 5 }))
      .build();
  }
}
