...
test.describe('Checkout page', () => {
  ...
  arrayForTest.forEach((testData) => {
    ...
    test(`Check products data and tax in Cart and place order for isEuropeanUnionCountry = ${testData.country.europeanUnion} and isSelfPickup = ${testData.isSelfPickup} `, async ({
      ...
      const shoppingCartInCheckoutPage = new ShoppingCartPage(newUserPage);
      await shoppingCartInCheckoutPage.checkProductInCart(productsForChecked);

      const orderTotalsInCheckoutPage = new OrderTotalsPage(newUserPage);
      await orderTotalsInCheckoutPage.checkOrderTotals(
        productsForChecked,
