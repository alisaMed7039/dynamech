...
import {
  ProductWithBrandAndName,
  WorkspacePage,
} from '../pages-and-components/pages/workspace.page';
...
test.describe('Confirmation page', () => {
  ...
  arrayForTest.push({
    country: randomElement(europeanUnionCountries),
    isSelfPickup: true,
  });
  arrayForTest.push({
    country: randomElement(notEuropeanUnionCountries),
    isSelfPickup: true,
  });
  ...
  arrayForTest.forEach((testData) => {
    test(`Check products data and prices in ConfirmationPage for isEuropeanUnionCountry = ${testData.country.europeanUnion} and isSelfPickup = true`, async ({
      ...
      const productSearchBlock = await workspacePage.choseWayToAddProduct(
        'product_search',
        userPage,
      );
      ...
      for await (let productForAddition of productsForAddition) {
        await productSearchBlock.searchProduct(productForAddition.productMpn);
        await productSearchBlock.checkSearchedProduct(true);
        await productSearchBlock.setQuantity(productForAddition.quantity);
        const productPrices = await productSearchBlock.choseRandomPrice();
        logger.info(`productPrices ${JSON.stringify(productPrices)}`);
        await productSearchBlock.addProductBtn.click();
        productsForChecked.unshift({
          ...productPrices,
          ...productForAddition,
          brand: '',
          name: '',
      ...
      const { checkoutPage, newUserPage } =
        await workspacePage.goToCheckoutAndAssertIt(userBrowserContext);
      ...
      await checkoutPage.setSelectValue(
        checkoutPage.billingAddressIdSelect,
        newAddressIdSelectOption,
      );
      ...
      const shoppingCartInConfirmationPage = new ShoppingCartPage(newUserPage);
      await shoppingCartInConfirmationPage.checkProductInCart(productsForChecked);

      const orderTotalsInConfirmationPage = new OrderTotalsPage(newUserPage);
      await orderTotalsInConfirmationPage.checkOrderTotals(
        productsForChecked,
        address.country.europeanUnion,
        testData.isSelfPickup,
      );
