...
export class ShoppingCartPage extends BaseComponent {
  ...
  constructor(page: Page) {
    super(page);
    this.page = page;
    this.shoppingCartTable = this.page
      .locator('div[class^="component_fix-box__"]')
      .filter({ hasText: 'Shopping cart' });
    this.productNameAndBrand = this.shoppingCartTable.locator(
      'div[class*="component_cart-product-list-item__cell_name__"]',
    );
    this.productPricesAndQuantity = this.shoppingCartTable.locator(
      'div[class*="component_cart-product-list-item__cell_price__"]',
    );
  }
  ...
  async checkProductInCart(products: ProductWithBrandAndName[]) {
    ...
    for (const productNameAndBrandCell of await this.productNameAndBrand.all()) {
      ...
      expect(textFromBrandLocator.trim().toLowerCase()).toEqual(
        productOfRow.brand.trim().toLowerCase(),
      );

      const totalPrice = await this.productPricesAndQuantity
        .nth(j)
        .locator('div')
        .nth(0)
        .textContent();
      expect(parseFloatPrice(parsePriceWithCurrencySymbol(totalPrice))).toEqual(
        parseFloatPrice(parsePriceWithCurrencySymbol(productOfRow.total)),
      );

      const quantityAndPrice = await this.productPricesAndQuantity
        .nth(j)
        .locator('div')
        .nth(1)
        .textContent();
