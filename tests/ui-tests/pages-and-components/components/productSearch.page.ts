...
export class ProductSearchPage extends BaseComponent {
  ...
  constructor(page: Page) {
    ...
    this.failMessage = this.searchedProduct
      .locator('div[class^="_notFound"] > div[class^="_state_"]')
      .filter({ hasText: 'Not found' });
    this.quantityTextbox = this.searchedProduct
      .locator('div')
      .filter({ hasText: /^Quantity/ })
      .getByRole('textbox');
    this.blocksOfPrice = this.searchedProduct.locator('div[class^="_sample_"]');
    this.addProductBtn = this.searchedProduct
      .locator('button')
      .filter({ hasText: 'Add to workspace' });
  }
