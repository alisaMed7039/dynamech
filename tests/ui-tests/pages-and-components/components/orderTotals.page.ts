...
export class OrderTotalsPage extends BaseComponent {
  ...
  constructor(page: Page) {
    super(page);
    this.page = page;
    this.cartTotalSub = this.page
      .getByTestId('cart-total-sub')
      .or(this.page.getByTestId('invoice-total-sub'));
    this.cartTax = this.page
      .getByTestId('cart-total-tax')
      .or(this.page.getByTestId('invoice-total-tax'));
    this.cartTotalGrand = this.page
      .getByTestId('cart-total-grand')
      .or(this.page.getByTestId('invoice-total-grand'));
  }
  ...
  async checkOrderTotals(
    products: ProductWithBrandAndName[],
    ...
    if (!isSelfPickup && isEuropeanUnion) {
      ...
      }).toPass({ intervals: [5000, 5000, 5000, 5000], timeout: 30000 });

      expect(formattedGrandTotal).toEqual(
        roundFloatPrice(formattedWorkspaceSubtotal + formattedTax),
      );
