...
export abstract class BaseComponent {
  ...
  async logCookie(): Promise<{
    storeFromPage: StorageState;
    cookiesFromPage: Array<Cookie>;
  }> {
    ...
    logger.info(
      `${this.getClassName()} this.page.context().storageState() ${JSON.stringify(storeFromPage)}`,
    );
    logger.info(
      `${this.getClassName()} this.page.context().cookies() ${JSON.stringify(cookiesFromPage)}`,
    );
  ...
  async isAuth(): Promise<boolean> {
    ...
    if (storeFromPage && storeFromPage?.cookies && Array.isArray(storeFromPage?.cookies)) {
      return (
        storeFromPage.cookies.some((cookie) => cookie.name === 'customer_token_hp') &&
  ...
  async fullyLoadedPage(): Promise<void> {
