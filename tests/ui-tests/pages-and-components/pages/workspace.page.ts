...
export class WorkspacePage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.subtotalQuantityValue = this.blockOfSubtotals
      .locator('div[class^="_total_"] > div[class^="_value_"]')
      .nth(0);
    this.subtotalPriceValue = this.blockOfSubtotals
      .locator('div[class^="_total_"] > div[class^="_value_"]')
      .nth(1);
  ...
  async choseWayToAddProduct(
    wayOfAdditionProduct: keyof typeof WAYS_OF_PRODUCT_ADDITION,
    userPage: Page,
  ) {
  ...
  async checkAddedProducts(products: ProductForAdd[]) {
    ...
    for await (let product of products) {
      ...
      await expect(this.getCellFromRow('Quantity', rowLocator)).toHaveValue(
  ...
  async checkTotalAmounts(products: ProductForAdd[]) {
    const subTotal = products.reduce(
      (acc, elm) => {
        ...
        acc.subtotalPrice = roundFloatPrice(
          acc.subtotalPrice + parseTotalPrice(elm.total).totalAmount,
        );
        return acc;
      },
    ...
    expect(parseFloatPrice(parsePriceWithCurrencySymbol(subtotalPriceValue))).toEqual(
  ...
  async goToCheckoutAndAssertIt(userBrowserContext: BrowserContext) {
    ...
    await checkoutPage.fullyLoadedPage();
