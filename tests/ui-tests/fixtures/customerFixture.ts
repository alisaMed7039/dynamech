import { Browser, BrowserContext, type Page, request, test as base } from '@playwright/test';
...
export const test = base.extend<Fixtures>({
  ...
  getAuthorizedUser: [
    async ({ getAuthFile, authorizedUserAmount }, use) => {
      ...
      for await (const userData of userDataArray) {
        ...
        const userBrowserContext = await browser.newContext({
          storageState: {
            ...
            origins: [
              {
                localStorage: [LOCALSTORAGE_FIELDS_FOR_AVOID__BANNER],
