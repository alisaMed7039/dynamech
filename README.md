...
Можно и нужно убрать work-around c LOCALSTORAGE_FIELDS_FOR_AVOID\_\_BANNER
из тела теста `tests/ui-tests/tests/auth.test.ts` в `customerFixture`. Но в рамках тестового пока оставим так.

2. А зачем эта писанина с LOCALSTORAGE_FIELDS_FOR_AVOID\_\_BANNER нельзя что ли крестик в тесте кликнуть?

Можно. Но это делается специально: work-around c LOCALSTORAGE_FIELDS_FOR_AVOID**BANNER отрабатывает в ui тестах
быстрее чем прокликивание. Плюс так лучше для масштабирования. Ведь по хорошему work-around c LOCALSTORAGE_FIELDS_FOR_AVOID**BANNER
